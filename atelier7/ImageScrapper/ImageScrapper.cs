...
namespace ImageScrapper
{
    class ImageScrapper
    {
        ...
        private void scraplink(Uri root, string sLink)
            {
                    ...
                    using (Stream s = wc.OpenRead(root))
                    {
                        using (StreamReader rdr = new StreamReader(s))
                        {
                            ...
                            foreach (Match img in this.regxImg.Matches(page))
                                ...
                                Task.Factory.StartNew(() => {
                                     Parallel.ForEach(this.regxLink.Matches(page).Cast<Match>(), link => scraplink(root, link.Groups[1].Value));
        ...
        private void DownloadImg(Uri root, string imgpath)
        {
            try
            {
                using (WebClient downloader = new WebClient())
                {
                    ...
                    {
                        downloader.DownloadFile(url,Path.Combine(this.imagedir,Path.GetFileName(imgpath)));
                    }
                }
            }
            catch (Exception x)
